...
in
{
  ...
  flake.modules.nixos.sam-home =
    ...
    let
      ...
      isManagedPersonal = enabled && host.platform != "wsl";
      isNativePersonal = enabled && host.platform != "wsl" && !isSteamDeck;
      hasUserSops = enabled && host.platform != "rpi";
      gpgPayload = import ./gpg/_content.nix {
      ...
      home-manager.users.${username} = {
        ...
        home = {
          ...
          packages =
            ...
            ++ lib.optionals (host.features.gui && host.platform != "wsl") [
              clementine
              discord
              ferdium
              plex-desktop
          ...
          persistence = lib.mkIf host.features.impermanence {
            "/persistent" = {
        ...
        programs = {
          ...
          gh = import ./github-cli/_content.nix;
          nix-index = {
            enable = true;
          ...
          gpg = lib.mkIf isManagedPersonal gpgPayload.programs.gpg;
          ...
          mcp = lib.mkIf host.features.personalMcp (import ./mcp/_personal-content.nix).programs.mcp;
          vim = lib.mkIf (host.platform != "wsl") (import ./vim/_content.nix { inherit pkgs; }).programs.vim;
          bash = import ./bash/_content.nix { inherit isSteamDeck nixProfile; };
          fish = {
            enable = true;
            generateCompletions = true;
            plugins = lib.mkIf (host.platform != "wsl") [
              {
                name = "fish-ssh-agent";
          ...
          atuin = lib.mkIf isNativePersonal (import ./atuin/_content.nix { inherit domain; }).programs.atuin;
          ...
          starship = (import ./starship/_content.nix { inherit lib; }).programs.starship;
          tmux = (import ./tmux/_content.nix).programs.tmux;
        };
        services.gpg-agent = lib.mkIf isManagedPersonal gpgPayload.services.gpg-agent;

        systemd.user.services.gpg-cleanup-stale-locks =
          lib.mkIf isManagedPersonal gpgPayload.systemd.user.services.gpg-cleanup-stale-locks;
