{ lib, pkgs, ... }:
{
  options.my.platform.steamdeck = {
    enable = lib.mkEnableOption "Steam Deck platform";
  ...
  config = {
    ...
    # footprint than the shared workstation defaults. Let `nh clean` own
    # generation cleanup so it does not conflict with `nix.gc`.
