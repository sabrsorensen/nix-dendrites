...
in
{
  ...
  home-manager.users.${username} = {
    ...
    sops.templates.nuget-higi-config =
      lib.mkIf (config.home-manager.users.${username}.sops.secrets ? nuget_higi_source_url)
        {
          path = "${homeDirectory}/.nuget/NuGet/NuGet.Config";
          mode = "0600";
          content = ''
            ...
            <configuration>
              <packageSources>
                ...
                <add key="higi" value="${
                  config.home-manager.users.${username}.sops.placeholder.nuget_higi_source_url
                }" />
              </packageSources>
            </configuration>
          '';
