        ...
        "netbird.{$DOMAIN}" = {
          ...
          routes = [
            ''
              ...
              reverse_proxy /management.ManagementService/* h2c://${managementAddress}:${toString managementPort} {
                transport http { read_timeout 0 write_timeout 0 dial_timeout 30s }
                flush_interval -1
              }
              reverse_proxy /ws-proxy/management/* http://${managementAddress}:${toString managementPort}
              reverse_proxy /signalexchange.SignalExchange/* h2c://${managementAddress}:${toString config.services.netbird.server.signal.port} {
                transport http { read_timeout 0 write_timeout 0 dial_timeout 30s }
                flush_interval -1
              }
